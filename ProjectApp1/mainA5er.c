...
void  insertintodelay(fptr f, int delay, int prio){
    ...
    int i=0;
		int j=0;
    for (i; i<delay_queue.CURRENTSIZE;i++){
        for (j; j<delay_queue.CURRENTSIZE;j++){
...
void QueTask(fptr f, int Priority)
{
    ...
    ready_queue.CURRENTSIZE++;
		...
		int z=0;
		int j=0;
    for ( z; z<ready_queue.CURRENTSIZE;z++){
        for (j; j<ready_queue.CURRENTSIZE;j++){
            if (ready_queue.tasks[z].priority > ready_queue.tasks[j].priority && z!=j){

                struct task temp = ready_queue.tasks[z];
	            ready_queue.tasks[z] = ready_queue.tasks[j];
...
void SystemClock_Config(void)
{
  ...
  */
	...
	//HAL_SYSTICK_Config(HAL_RCC_GetHCLKFreq()*2); //0.05 represets 1 tick (50ms)
